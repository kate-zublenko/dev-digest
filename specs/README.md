...
feature: `NN-feature-name.md`. Work that lives inside a single package goes in that
package's `specs/` instead — and for `e2e/`, prose specs go in `e2e/docs/`, because
`e2e/specs/` is already taken by executable flow files.
...
explanation into `docs/`. Stale specs are worse than missing ones — an agent reads
